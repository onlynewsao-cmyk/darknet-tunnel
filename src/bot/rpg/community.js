...
async function addAllUsersToMainGroup(sock, ownerJid, mainType) {
  ...
  const results = { added: [], invited: [], errors: [], jaEstava: [], naoEntraram: [], group: null };
  ...
  for (const user of users) {
    ...
    try {
      ...
      else if (st === '408') { results.naoEntraram.push({ user: user.name || num, motivo: 'saiu recente (408) — só re-add após 24h' }); }
      else if (st === '403') { results.naoEntraram.push({ user: user.name || num, motivo: 'privacidade (403) — manda .invite no PV' }); }
      else if (st) { results.naoEntraram.push({ user: user.name || num, motivo: 'status ' + st }); }
    } catch (e) {
      results.naoEntraram.push({ user: user.name || num, motivo: e.message?.slice(0, 40) || 'erro' });
    }
