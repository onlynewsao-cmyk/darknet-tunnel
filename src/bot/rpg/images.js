...
async function generateProfileImage(player) {
  // v9.22: corrigir propriedades — o RPGPlayer schema usa
  // race/coins/mp/maxMp/stats.str/dex/int/vit/luk/equipment
  // (antes usava origin/berries/energy/max_energy/stat_* que NÃO existem)
  const rpg = require('./engine');
  const rank = rpg.getRank(player.level || 1);
  const rankColors = {
    'E': '#888888', 'D': '#22cc44', 'C': '#4488ff', 'B': '#aa44ff',
    'A': '#ffcc00', 'S': '#ff4444', 'SS': '#ff8800', 'SSS': '#ff00ff',
  };
  const color = rankColors[rank.name] || '#888888';
  ...
  const race = player.race || 'humano';
  const cls = player.class || 'guerreiro';
  const emoji = originEmojis[race] || '⚔️';
  const stats = player.stats || { str: 6, dex: 6, int: 6, vit: 6, luk: 6 };
  const eq = player.equipment || {};
  const weaponName = eq.weapon || 'Nenhuma arma equipada';
  ...
  const svg = `<svg width="600" height="400" xmlns="http://www.w3.org/2000/svg">
    ...
    <text x="60" y="70" text-anchor="middle" fill="${color}" font-size="28" font-weight="bold">${rank.name}</text>
    
    <!-- Name -->
    <text x="120" y="50" fill="#ffffff" font-size="22" font-weight="bold">${player.name || 'Aventureiro'}</text>
    <text x="120" y="72" fill="#aaaaaa" font-size="13">${emoji} ${race} ${cls} • Nível ${player.level || 1}</text>
    
    <!-- HP Bar -->
    <text x="30" y="120" fill="#ff4444" font-size="12">❤️ HP</text>
    <rect x="80" y="110" width="480" height="16" rx="8" fill="#1a1a1a"/>
    <rect x="80" y="110" width="${Math.round(480 * Math.max(0, Math.min(1, (player.hp || 0) / Math.max(1, player.maxHp || 150))))}" height="16" rx="8" fill="#ff4444"/>
    <text x="570" y="123" text-anchor="end" fill="#ffffff" font-size="11">${player.hp || 0}/${player.maxHp || 150}</text>
    
    <!-- MP Bar -->
    <text x="30" y="150" fill="#4488ff" font-size="12">💙 MP</text>
    <rect x="80" y="140" width="480" height="16" rx="8" fill="#1a1a1a"/>
    <rect x="80" y="140" width="${Math.round(480 * Math.max(0, Math.min(1, (player.mp || 0) / Math.max(1, player.maxMp || 80))))}" height="16" rx="8" fill="#4488ff"/>
    <text x="570" y="153" text-anchor="end" fill="#ffffff" font-size="11">${player.mp || 0}/${player.maxMp || 80}</text>
    
    <!-- Stats -->
    <text x="30" y="195" fill="#ffffff" font-size="14" font-weight="bold">📊 Atributos</text>
    <text x="30"  y="220" fill="#ff8888" font-size="12">💪 STR: ${stats.str ?? 6}</text>
    <text x="170" y="220" fill="#88ff88" font-size="12">🏃 DEX: ${stats.dex ?? 6}</text>
    <text x="310" y="220" fill="#8888ff" font-size="12">🔮 INT: ${stats.int ?? 6}</text>
    <text x="450" y="220" fill="#ffff88" font-size="12">🛡️ VIT: ${stats.vit ?? 6}</text>
    <text x="30" y="245" fill="#aaffaa" font-size="12">🍀 LUK: ${stats.luk ?? 6}</text>
    
    <!-- Economy -->
    <text x="30"  y="280" fill="#ffcc00" font-size="13">💰 ${(player.coins || 0).toLocaleString()} Coins</text>
    <text x="250" y="280" fill="#4488ff" font-size="13">🏦 ${(player.bank || 0).toLocaleString()} Banco</text>
    <text x="430" y="280" fill="#ff4444" font-size="13">⭐ ${(player.reputation || 0).toLocaleString()} Rep</text>
    
    <!-- Weapon -->
    <text x="30" y="320" fill="#ffffff" font-size="14" font-weight="bold">⚔️ Arma Equipada</text>
    <text x="30" y="342" fill="#aaaaaa" font-size="12">${weaponName}</text>
    
    <!-- Kills/Deaths -->
    <text x="30" y="375" fill="#ffffff" font-size="13">⚔️ ${player.kills || 0} kills · 💀 ${player.deaths || 0} mortes · ♥️ ${player.lives ?? 3} vidas</text>
  </svg>`;
